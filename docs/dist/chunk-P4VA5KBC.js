import{e as F}from"./chunk-RBEKY5OM.js";import{E,G as S,H as V,I as L,J as I,a as $,d as o}from"./chunk-PRFEE4R6.js";var G="kanau-playlists",U="kanau-music-videos-cache-v2",j=24,f="all-streams",k=1,H="newest",_="grid",m=null,T=null,C=!1,B="",R=!1;function g(){try{return JSON.parse(localStorage.getItem(G)||"[]")}catch{return[]}}function M(t){try{localStorage.setItem(G,JSON.stringify(t))}catch{}}function W(t){let a=g(),s={id:String(Date.now()),name:t.trim(),createdAt:new Date().toISOString(),streams:[]};return a.unshift(s),M(a),s}function nt(t){M(g().filter(a=>a.id!==t))}function Y(t,a){let s=g(),e=s.find(l=>l.id===t);return!e||e.streams.includes(a)?!1:(e.streams.push(a),M(s),!0)}function it(t,a){let s=g(),e=s.find(l=>l.id===t);e&&(e.streams=e.streams.filter(l=>l!==a),M(s))}function Lt(t){return g().some(a=>a.streams.includes(t))}function w(){let t=$("#panel-playlists");if(!t)return;let a=F.data?.streams||[];t.innerHTML=`
    <div class="pl-wrap">
      ...
      <div class="pl-subtab-body" id="pl-subtab-body">
        ${f==="all-streams"?Z(a,k):f==="music"?ot():$t(a)}
      </div>
    </div>
  `,t.onclick=s=>{let e=s.target.closest("[data-pl-subtab]");if(e){f=e.dataset.plSubtab,f==="all-streams"&&(k=1),w(),f==="music"&&dt();return}let l=s.target.closest("[data-pl-sort]");if(l){H=l.dataset.plSort,k=1,K(a);return}let d=s.target.closest("[data-pl-page]");if(d&&!d.disabled){k=Number(d.dataset.plPage),K(a);return}let v=s.target.closest("[data-music-view]");if(v){_=v.dataset.musicView,tt();return}let h=s.target.closest("[data-play-music]");if(h&&m?.length){let r=Number(h.dataset.playMusic);import("./chunk-JRCBHL36.js").then(n=>n.playMusicQueue(m,r));return}let c=s.target.closest("[data-watch-music]");if(c&&m?.length){let r=Number(c.dataset.watchMusic),n=m[r];n?.url&&window.__openStreamViewer?.({url:n.url,title:n.title,isMv:!0});return}let i=s.target.closest("[data-playlist-add-mv]");if(i){let r=i.dataset.playlistAddMv,n=i.dataset.streamTitle||"";St("mv:"+r,n);return}f==="my-playlists"&&_t(s,a)},t.oncompositionstart=s=>{s.target.closest("#pl-music-search")&&(R=!0)},t.oncompositionend=s=>{let e=s.target.closest("#pl-music-search");e&&(R=!1,requestAnimationFrame(()=>D(e)))},t.oninput=s=>{let e=s.target.closest("#pl-music-search");e&&(B=e.value||"",!(R||s.isComposing)&&D(e))},t.addEventListener("error",s=>{let e=s.target;if(!e.classList.contains("pl-sg-thumb"))return;let l=e.dataset.fallback;l&&e.src!==l&&(e.src=l,delete e.dataset.fallback)},!0)}function D(t){B=t.value||"",tt()}var rt=[{key:"newest",label:"\u65B0\u3057\u3044\u9806"},{key:"oldest",label:"\u53E4\u3044\u9806"},{key:"most-songs",label:"\u66F2\u6570\u2193"},{key:"fewest-songs",label:"\u66F2\u6570\u2191"}];function ct(t,a){let s=t.slice();return a==="oldest"?s.reverse():a==="most-songs"?s.sort((e,l)=>(l.songs?.length??0)-(e.songs?.length??0)):a==="fewest-songs"?s.sort((e,l)=>(e.songs?.length??0)-(l.songs?.length??0)):s}function Z(t,a){if(!t.length)return`
      <div class="pl-empty-state">
        <p>\u914D\u4FE1\u30C7\u30FC\u30BF\u3092\u8AAD\u307F\u8FBC\u3093\u3067\u3044\u307E\u3059\u2026</p>
        <p class="pl-empty-hint">\u5148\u306B\u30BF\u30A4\u30E0\u30E9\u30A4\u30F3\u30BF\u30D6\u3092\u958B\u304F\u3068\u3059\u3050\u306B\u8868\u793A\u3055\u308C\u307E\u3059</p>
      </div>`;let s=ct(t,H),e=s.length,l=Math.max(1,Math.ceil(e/j)),d=Math.min(Math.max(1,a),l),v=(d-1)*j,c=s.slice(v,v+j).map(n=>{let y=S(n),p=L(n.url),u=I(n.url),b=n.songs?.length??0;return`
    ...
    </div>`}<div class="pl-stream-grid" id="pl-stream-grid">${c}</div>${i}`}function K(t){let a=$("#pl-subtab-body");if(!a){w();return}a.innerHTML=Z(t,k);let s=$("#panel-playlists");s&&s.addEventListener("error",e=>{let l=e.target;if(!l.classList.contains("pl-sg-thumb"))return;let d=l.dataset.fallback;d&&l.src!==d&&(l.src=d,delete l.dataset.fallback)},{once:!0,capture:!0}),a.scrollIntoView({behavior:"smooth",block:"start"})}function ot(){let t=O();return m===null&&t.length&&(m=t),C=!0,A(m||[])}async function dt(){if(m===null){let a=O();if(a.length){m=a;let s=$("#pl-subtab-body");s&&f==="music"&&(s.innerHTML=A(m))}else{m=[];let s=$("#pl-subtab-body");s&&f==="music"&&(s.innerHTML=A(m))}C=!0,m=await mt(),C=!1}let t=$("#pl-subtab-body");t&&f==="music"&&(t.innerHTML=A(m))}function A(t){return ut(t)+`<div id="pl-music-results">${X(t)}</div>`}function ut(t){let a=N(),e=z(t).length;return`
    <div class="pl-music-viewbar">
      ...
      <span class="pl-music-count">${e}${e===t.length?"":` / ${t.length}`}\u4EF6</span>
    ...
    </div>`}function X(t){let a=z(t);return C&&!t.length?'<div class="pl-empty-state"><p>\u8AAD\u307F\u8FBC\u307F\u4E2D\u2026</p><p class="pl-empty-hint">\u691C\u7D22\u6B04\u306F\u3053\u306E\u307E\u307E\u5165\u529B\u3067\u304D\u307E\u3059</p></div>':t.length?a.length?_==="grid"?J(a):_==="list"?gt(a):_==="category"?yt(a):J(a):C?`<div class="pl-empty-state"><p>\u6700\u65B0\u30C7\u30FC\u30BF\u3092\u78BA\u8A8D\u4E2D\u2026</p><p class="pl-empty-hint">\u300C${o(N())}\u300D\u306E\u5019\u88DC\u3092\u8AAD\u307F\u8FBC\u3093\u3067\u3044\u307E\u3059</p></div>`:'<div class="pl-empty-state"><p>\u4E00\u81F4\u3059\u308B\u52D5\u753B\u304C\u3042\u308A\u307E\u305B\u3093</p><p class="pl-empty-hint">\u300C\u66F2\u540D / \u30A2\u30FC\u30C6\u30A3\u30B9\u30C8\u300D\u306E\u3088\u3046\u306B\u533A\u5207\u3063\u3066\u691C\u7D22\u3067\u304D\u307E\u3059</p></div>':'<div class="pl-empty-state"><p>\u52D5\u753B\u304C\u767B\u9332\u3055\u308C\u3066\u3044\u307E\u305B\u3093</p><p class="pl-empty-hint">\u7BA1\u7406\u753B\u9762\u304B\u3089\u767B\u9332\u3067\u304D\u307E\u3059</p></div>'}function N(){let t=$("#pl-music-search");return t&&(B=t.value||""),B}function tt(){let t=m||[],a=$(".pl-music-count");if(a){let e=z(t).length;a.textContent=`${e}${e===t.length?"":` / ${t.length}`}\u4EF6`}document.querySelectorAll("[data-music-view]").forEach(e=>{e.classList.toggle("active",e.dataset.musicView===_)});let s=$("#pl-music-results");s&&(s.innerHTML=X(t))}function O(){try{let t=JSON.parse(localStorage.getItem(U)||"null");return Array.isArray(t?.videos)?t.videos:[]}catch{return[]}}function pt(t){try{localStorage.setItem(U,JSON.stringify({videos:t,cachedAt:Date.now()}))}catch{}}async function mt(){return T||(T=fetch("/data/music.json",{cache:"no-store"}).then(t=>t.ok?t.json():Promise.reject(new Error(`music.json ${t.status}`))).then(t=>{let a=Array.isArray(t?.videos)?t.videos:[];return pt(a),a}).catch(()=>m||O()),T)}function st(t){return String(t||"").normalize("NFKC").toLowerCase().replace(/[！-～]/g,a=>String.fromCharCode(a.charCodeAt(0)-65248)).replace(/[‐-‒–—―ー]/g,"-").replace(/\s+/g," ").trim()}function bt(t){return st(t).split(/[\/／|｜\s]+/).map(a=>a.trim()).filter(Boolean)}function vt(t){let a=t.title||"",s=a.split(/[\/／|｜]/).map(l=>l.trim()).filter(Boolean),e=x(t).label;return st([a,...s,t.originalArtist,t.character,t.type,e].filter(Boolean).join(" "))}function z(t){let a=bt(N()),s=t.map((e,l)=>({v:e,i:l}));return a.length?s.filter(({v:e})=>{let l=vt(e);return a.every(d=>l.includes(d))}):s}function x(t){switch(t.type){case"cover":return{label:"\u30AB\u30D0\u30FC",cls:"mv-badge-cover",sub:t.originalArtist||"\u30AB\u30D0\u30FC\u66F2"};case"office":return{label:"Re:AcT",cls:"mv-badge-office",sub:"Re:AcT"};case"character":return{label:"\u30AD\u30E3\u30E9",cls:"mv-badge-character",sub:t.character||"\u30AD\u30E3\u30E9\u30BD\u30F3"};default:return{label:"\u30AA\u30EA\u30B8\u30CA\u30EB",cls:"mv-badge-original",sub:"\u304B\u306A\u3046"}}}function at(t,a){let s=L(t.url),e=I(t.url),{label:l,cls:d,sub:v}=x(t);return`
    <div class="mv-card">
    ...
    </div>`}function ft(t,a){let{label:s,cls:e,sub:l}=x(t);return`
    <div class="mv-list-row">
      ...
      <button class="mv-add-btn" type="button"
        ...
        data-stream-title="${o(t.title||"")}"
        title="\u30D7\u30EC\u30A4\u30EA\u30B9\u30C8\u306B\u8FFD\u52A0">\uFF0B</button>
    </div>`}function J(t){return`<div class="mv-grid">${t.map(({v:a,i:s})=>at(a,s)).join("")}</div>`}function gt(t){return`<div class="mv-list">${t.map(({v:a,i:s})=>ft(a,s)).join("")}</div>`}function yt(t){return`
    <div class="mv-category">
      ...
      <div class="mv-cat-section">
        ...
        <div class="mv-grid">${e.map(({v:l,i:d})=>at(l,d)).join("")}</div>
      </div>`).join("")}
    </div>`}function Tt(){return m||[]}function ht(t){if(!t?.startsWith("mv:"))return null;let a=t.slice(3);return(m||[]).find(s=>s.id===a)||null}function $t(t){let a=g();return a.length?`
    ...
    <div class="pl-grid">
      ...
      <div class="pl-sort-btns">
        ...
        </div>`;let{label:P,sub:et}=x(r),lt=r.type||"original",q=(m||[]).indexOf(r);return`
        <div class="pl-stream-row">
          ${u}
          <div class="pl-stream-info">
            <span class="pl-stream-date"><span class="mv-badge-inline mv-type-${lt}">${P}</span></span>
            <span class="pl-stream-title">${o(r.title||"\u2014")}</span>
            <span class="pl-stream-meta">${o(et)}</span>
          </div>
    ...
    <div class="pl-card">
      ...
      </div>`,s.hidden=!1,s.querySelector("#pl-modal-close").addEventListener("click",d),s.querySelector("#pl-modal-backdrop").addEventListener("click",d),s.querySelector("#pl-modal-new").addEventListener("click",()=>{let c=prompt("\u30D7\u30EC\u30A4\u30EA\u30B9\u30C8\u540D")?.trim();if(!c)return;let i=W(c);Y(i.id,t),d(),Q(`\u300C${c}\u300D\u306B\u8FFD\u52A0\u3057\u307E\u3057\u305F`)}),s.querySelectorAll("[data-pl-add]:not([disabled])").forEach(c=>{c.addEventListener("click",()=>{let i=c.dataset.plAdd,r=g().find(n=>n.id===i);Y(i,t),l(),Q(`\u300C${r?.name}\u300D\u306B\u8FFD\u52A0\u3057\u307E\u3057\u305F`)})})},d=()=>{s.hidden=!0};l(),document.addEventListener("keydown",function v(h){h.key==="Escape"&&(d(),document.removeEventListener("keydown",v))})}function Q(t){let a=$("#pl-toast");a||(a=document.createElement("div"),a.id="pl-toast",document.body.appendChild(a)),a.textContent=t,a.classList.add("pl-toast--show"),clearTimeout(a._timer),a._timer=setTimeout(()=>a.classList.remove("pl-toast--show"),2500)}export{Y as addStreamToPlaylist,W as createPlaylist,nt as deletePlaylist,Tt as getMusicVideos,g as getPlaylists,Lt as isStreamInAnyPlaylist,it as removeStreamFromPlaylist,w as renderPlaylists,ht as resolveMusicVideoId,St as showAddToPlaylistModal};

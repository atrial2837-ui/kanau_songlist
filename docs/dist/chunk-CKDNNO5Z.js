...
`).map(b=>b.trim()).filter(Boolean),o=0;for(let b of c){let x=Gt(b);if(!x)continue;let _=Qt(x.title,x.artist,t.songs);if(_>=0){let w=l.querySelector(`[data-bulk-ts-idx="${_}"]`);w&&(w.value=x.start,o++)}}let v=l.querySelector(".sv-paste-result");v&&(v.textContent=o>0?`${c.length}\u884C\u3092\u89E3\u6790 \u2192 ${o}\u66F2\u306B\u5165\u529B\u3057\u307E\u3057\u305F`:"\u4E00\u81F4\u3059\u308B\u66F2\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F",v.hidden=!1)}),l.querySelector(".sv-bulk-rows").addEventListener("click",d=>{let c=d.target.closest("[data-bulk-now]");if(!c)return;let o=parseInt(c.dataset.bulkNow,10),v=p?.getCurrentTime?.();if(v!=null){let b=l.querySelector(`[data-bulk-ts-idx="${o}"]`);b&&(b.value=U(Math.floor(v)))}}),l.querySelector("#sv-bulk-submit").addEventListener("click",async()=>{let d=l.querySelector("#sv-bulk-note").value.trim()||null,c=l.querySelector("#sv-bulk-status"),o=l.querySelector("#sv-bulk-submit"),v=[];if(l.querySelectorAll("[data-bulk-ts-idx]").forEach(_=>{let w=parseInt(_.dataset.bulkTsIdx,10),g=mt(_.value.trim());g!==null&&v.push({songIndex:w,timeSeconds:g})}),!v.length){c.textContent="\u30BF\u30A4\u30E0\u30B9\u30BF\u30F3\u30D7\u304C1\u3064\u3082\u5165\u529B\u3055\u308C\u3066\u3044\u307E\u305B\u3093",c.className="sv-cts-modal-status error",c.hidden=!1;return}o.disabled=!0,o.textContent=`\u7533\u8ACB\u4E2D\u2026 (0/${v.length})`,c.hidden=!0;let b=0,x=0;await Promise.all(v.map(async _=>{try{(await St(t.channel,t.index,{songIndex:_.songIndex,timeSeconds:_.timeSeconds,submitterNote:d})).ok?b++:x++}catch{x++}o.textContent=`\u7533\u8ACB\u4E2D\u2026 (${b+x}/${v.length})`})),x===0?(c.textContent=`${b}\u66F2\u5206\u306E\u30BF\u30A4\u30E0\u30B9\u30BF\u30F3\u30D7\u3092\u7533\u8ACB\u3057\u307E\u3057\u305F\uFF01\u5BE9\u67FB\u5F8C\u306B\u516C\u958B\u3055\u308C\u307E\u3059\u3002`,c.className="sv-cts-modal-status success",o.hidden=!0,l.querySelector(".sv-cts-modal-cancel").textContent="\u9589\u3058\u308B"):(c.textContent=`${b}\u4EF6\u6210\u529F / ${x}\u4EF6\u5931\u6557\u3002\u5931\u6557\u5206\u3092\u518D\u8A66\u884C\u3057\u3066\u304F\u3060\u3055\u3044\u3002`,c.className="sv-cts-modal-status error",o.disabled=!1,o.textContent="\u4E00\u62EC\u7533\u8ACB\u3059\u308B"),c.hidden=!1}),document.addEventListener("keydown",function d(c){c.key==="Escape"&&(u(),document.removeEventListener("keydown",d))})}var At="kanauViewerQueueCollapsed",S=null,ut=!1;function st(t){let e=S,s=e?.items?.[t];if(s){e.idx=t,ut=!0;try{s.kind==="mv"?M({url:s.video.url,title:s.video.title,isMv:!0}):M(s.stream)}finally{ut=!1}}}function Re(t){t?.items?.length&&(S={name:t.name||"\u30DE\u30A4\u30EA\u30B9\u30C8",items:t.items,idx:0,repeat:localStorage.getItem("kanauListRepeat")==="1",collapsed:localStorage.getItem(At)==="1"},st(Math.max(0,Math.min(t.idx||0,t.items.length-1))))}function $s(t,e=0,s=0){if(!t?.length)return!1;let a=t.filter(i=>i?.url).map((i,l)=>i._stream?{kind:"stream",key:i._stream.url||`stream:${l}`,stream:i._stream}:{kind:"mv",key:`mv:${L(i.url)||l}`,video:{...i,isMv:!0}});if(!a.length)return!1;S={name:"\u97F3\u697D\u30D7\u30EC\u30A4\u30E4\u30FC\u306E\u30AD\u30E5\u30FC",items:a,idx:Math.max(0,Math.min(e,a.length-1)),repeat:localStorage.getItem("kanauListRepeat")==="1",collapsed:localStorage.getItem(At)==="1"};let n=S.items[S.idx];ut=!0;try{n.kind==="mv"?M({...n.video,isMv:!0},s):M(n.stream,s)}finally{ut=!1}return!0}function qt(){let t=S;if(!t?.items?.length)return"";let e=t.items[t.idx],s=e?.kind==="mv"?e.video?.title||"\u52D5\u753B":e?.stream?.title||"\u914D\u4FE1";return`
  ...
  `,ze(u),e.onchange=o=>{let v=o.target.closest("#sv-ap-check"),b=o.target.closest("#sv-repeat-check");if(v){Y=v.checked;let x=v.closest(".sv-bp-ap-switch");x&&x.classList.toggle("sv-bp-ap-switch--on",Y)}if(b){D=b.checked;let x=b.closest(".sv-bp-ap-switch");x&&x.classList.toggle("sv-bp-ap-switch--on",D)}},e.onclick=o=>{if(ve(o))return;let v=o.target.closest("[data-bp-action]");if(!v)return;let b=v.dataset.bpAction;if(b==="open-stream"){let x=v.dataset.bpChannel,_=parseInt(v.dataset.bpIndex,10),w=(C.data?.streams||[]).find(g=>g.channel===x&&g.index===_);w&&M(w)}else b==="prev-stream"?Oe():b==="next-stream"?pe():b==="toggle-play"?he():b==="bookmark-stream"&&we(d,t.title||"\u914D\u4FE1",v)},Ht(e);try{let o=(p||y)?.getPlayerState?.()===window.YT?.PlayerState?.PLAYING;yt(o)}catch{}}var lt=null;async function Nt(){if(lt)return lt;try{lt=(await(await fetch("data/music.json")).json())?.videos||[]}catch{lt=[]}return lt}function Ge(t){let e=String(t||"");return e=e.replace(/【[^】]*】/g," "),e=e.replace(/^\s*MV[⌇|｜♪♬:：\-\s]*/i," "),e=e.split(/[\/／|｜]/)[0],e=e.replace(/歌ってみた|covered?\s*(by.*)?$/gi," "),e.trim()}async function Qe(t){let e=r("#sv-below-player");if(!e)return;try{await q.ensureFullData()}catch{}let s=await Nt();if(r("#stream-viewer")?._currentStream!==t)return;let a=C.channelData?.combined?.streams||C.data?.streams||[],n=R(Ge(t.title)),i=[];if(n.length>1)for(let f of a){let $=(f.songs||[]).find(E=>{let I=R(E.title);return I===n||I.length>1&&(I.includes(n)||n.includes(I))});$&&i.push({stream:f,songTitle:$.title})}let l=i.slice(0,8),u={original:"\u30AA\u30EA\u30B8\u30CA\u30EB",office:"Re:AcT",character:"\u30AD\u30E3\u30E9\u30BD\u30F3",cover:"\u30AB\u30D0\u30FC"},d=s.find(f=>f.url===t.url),c=s.filter(f=>f.url!==t.url).sort((f,$)=>{let E=d&&f.type===d.type?1:0,I=d&&$.type===d.type?1:0;return E!==I?I-E:($.publishedAt||"").localeCompare(f.publishedAt||"")}).slice(0,12),o=s.findIndex(f=>L(f.url)===L(t.url)),v=o>=0&&o<s.length-1?s[o+1]:null,b=o>0?s[o-1]:null,x=d||s.find(f=>L(f.url)===L(t.url)),_=x?"mv:"+x.id:"",w=_?ge(_):!1,g=S,k=!!g?.items?.length,j=k&&g.idx>0||!!b,at=k&&g.idx<g.items.length-1||!!v;e.innerHTML=`
    <div class="sv-bp-wrap">
      ${qt()}
      ...
      <div class="sv-bp-section">
        ...
        <div class="sv-bp-related-list">
          ${l.map(f=>{let $=it(f.stream.url);return`<button class="sv-bp-rel-card" type="button" data-mv-action="open-stream" data-mv-channel="${m(f.stream.channel)}" data-mv-index="${f.stream.index}">
              ...
              <div class="sv-bp-rel-info">
                <div class="sv-bp-rel-title">${m(f.stream.title||"\u914D\u4FE1")}</div>
                <div class="sv-bp-rel-meta">${z(f.stream.date)}\u3000\u7B2C${f.stream.index}\u67A0</div>
                <div class="sv-bp-rel-songs">${h("music")} ${m(f.songTitle)}</div>
              </div>
            </button>`}).join("")}
        </div>
      </div>
      ...
      <div class="sv-bp-section">
        ...
        <div class="sv-mv-grid">
          ${c.map(f=>{let $=it(f.url);return`<button class="sv-mv-card" type="button" data-mv-action="open-mv" data-mv-url="${m(f.url)}" data-mv-title="${m(f.title)}">
              ...
              <div class="sv-mv-card-body">
                <div class="sv-mv-card-title">${m(f.title)}</div>
  ...
  `,e.onchange=f=>{let $=f.target.closest("#sv-ap-check"),E=f.target.closest("#sv-repeat-check");if($){Y=$.checked;let I=$.closest(".sv-bp-ap-switch");I&&I.classList.toggle("sv-bp-ap-switch--on",Y)}if(E){D=E.checked;let I=E.closest(".sv-bp-ap-switch");I&&I.classList.toggle("sv-bp-ap-switch--on",D)}},e.onclick=f=>{if(ve(f))return;let $=f.target.closest("[data-mv-action]");if(!$)return;let E=$.dataset.mvAction;if(E==="open-stream"){let I=$.dataset.mvChannel,$e=parseInt($.dataset.mvIndex,10),Rt=(C.channelData?.combined?.streams||C.data?.streams||[]).find(Bt=>Bt.channel===I&&Bt.index===$e);Rt&&M(Rt)}else E==="open-mv"?M({url:$.dataset.mvUrl,title:$.dataset.mvTitle,isMv:!0}):E==="all-videos"?q.activateTab("playlists"):E==="toggle-play"?he():E==="mv-prev"?k&&g.idx>0?st(g.idx-1):Be(t):E==="mv-next"?k&&g.idx<g.items.length-1?st(g.idx+1):me(t):E==="bookmark-mv"&&we($.dataset.mvKey,t.title||"\u52D5\u753B",$)},Ht(e);try{let f=(p||y)?.getPlayerState?.()===window.YT?.PlayerState?.PLAYING;yt(f)}catch{}}function tt(t,e,s,a){t.innerHTML=e.map((n,i)=>Ae(n,i,s,a)).join("")}function We(){if(r("#stream-viewer"))return;let t=r("#panel-player");if(!t)return;let e=document.createElement("div");e.id="stream-viewer",e.hidden=!0,e.setAttribute("aria-label","\u914D\u4FE1\u30D7\u30EC\u30A4\u30E4\u30FC"),e.innerHTML=`

import{e as O}from"./chunk-N3S3MIXU.js";import{E as T,G as S,H as x,I as L,J as B,a as h,d as c}from"./chunk-FKVR6ZKV.js";var Y="kanau-playlists",E=24,g="all-streams",k=1,I="newest",_="grid",$=null;function v(){try{return JSON.parse(localStorage.getItem(Y)||"[]")}catch{return[]}}function M(t){try{localStorage.setItem(Y,JSON.stringify(t))}catch{}}function q(t){let s=v(),a={id:String(Date.now()),name:t.trim(),createdAt:new Date().toISOString(),streams:[]};return s.unshift(a),M(s),a}function K(t){M(v().filter(s=>s.id!==t))}function V(t,s){let a=v(),l=a.find(e=>e.id===t);return!l||l.streams.includes(s)?!1:(l.streams.push(s),M(a),!0)}function J(t,s){let a=v(),l=a.find(e=>e.id===t);l&&(l.streams=l.streams.filter(e=>e!==s),M(a))}function dt(t){return v().some(s=>s.streams.includes(t))}function w(){let t=h("#panel-playlists");if(!t)return;let s=O.data?.streams||[];t.innerHTML=`
    <div class="pl-wrap">
  ...
  `,t.onclick=a=>{let l=a.target.closest("[data-pl-subtab]");if(l){g=l.dataset.plSubtab,g==="all-streams"&&(k=1),w(),g==="music"&&Z();return}let e=a.target.closest("[data-pl-sort]");if(e){I=e.dataset.plSort,k=1,R(s);return}let d=a.target.closest("[data-pl-page]");if(d&&!d.disabled){k=Number(d.dataset.plPage),R(s);return}let b=a.target.closest("[data-music-view]");if(b){_=b.dataset.musicView;let n=h("#pl-subtab-body");n&&$&&(n.innerHTML=D($));return}let y=a.target.closest("[data-play-music]");if(y&&$?.length){let n=Number(y.dataset.playMusic);import("./chunk-VBV6MTL6.js").then(i=>i.playMusicQueue($,n));return}let r=a.target.closest("[data-playlist-add-mv]");if(r){let n=r.dataset.playlistAddMv,i=r.dataset.streamTitle||"";rt("mv:"+n,i);return}g==="my-playlists"&&nt(a,s)},t.addEventListener("error",a=>{let l=a.target;if(!l.classList.contains("pl-sg-thumb"))return;let e=l.dataset.fallback;e&&l.src!==e&&(l.src=e,delete l.dataset.fallback)},!0)}var Q=[{key:"newest",label:"\u65B0\u3057\u3044\u9806"},{key:"oldest",label:"\u53E4\u3044\u9806"},{key:"most-songs",label:"\u66F2\u6570\u2193"},{key:"fewest-songs",label:"\u66F2\u6570\u2191"}];function U(t,s){let a=t.slice();return s==="oldest"?a.reverse():s==="most-songs"?a.sort((l,e)=>(e.songs?.length??0)-(l.songs?.length??0)):s==="fewest-songs"?a.sort((l,e)=>(l.songs?.length??0)-(e.songs?.length??0)):a}function z(t,s){if(!t.length)return`
      <div class="pl-empty-state">
        <p>\u914D\u4FE1\u30C7\u30FC\u30BF\u3092\u8AAD\u307F\u8FBC\u3093\u3067\u3044\u307E\u3059\u2026</p>
        <p class="pl-empty-hint">\u5148\u306B\u30BF\u30A4\u30E0\u30E9\u30A4\u30F3\u30BF\u30D6\u3092\u958B\u304F\u3068\u3059\u3050\u306B\u8868\u793A\u3055\u308C\u307E\u3059</p>
      </div>`;let a=U(t,I),l=a.length,e=Math.max(1,Math.ceil(l/E)),d=Math.min(Math.max(1,s),e),b=(d-1)*E,r=a.slice(b,b+E).map(o=>{let f=S(o),u=L(o.url),p=B(o.url),m=o.songs?.length??0;return`
      <button class="pl-sg-card" type="button" data-stream-play="${c(f)}"
        ...
        <div class="pl-sg-info">
          ...
          <span class="pl-sg-date">${c(T(o.date)||"")}</span>
        </div>
      </button>`}).join(""),n=e>1?`
    ...
    <div class="pl-sort-bar">
      ${Q.map(o=>`
        <button class="pl-sort-opt${I===o.key?" active":""}"
          data-pl-sort="${o.key}" type="button">${o.label}</button>`).join("")}
    </div>`}<div class="pl-stream-grid" id="pl-stream-grid">${r}</div>${n}`}function R(t){let s=h("#pl-subtab-body");if(!s){w();return}s.innerHTML=z(t,k);let a=h("#panel-playlists");a&&a.addEventListener("error",l=>{let e=l.target;if(!e.classList.contains("pl-sg-thumb"))return;let d=e.dataset.fallback;d&&e.src!==d&&(e.src=d,delete e.dataset.fallback)},{once:!0,capture:!0}),s.scrollIntoView({behavior:"smooth",block:"start"})}function W(){return'<div class="pl-empty-state"><p>\u8AAD\u307F\u8FBC\u307F\u4E2D\u2026</p></div>'}async function Z(){if($===null)try{$=(await(await fetch("/data/music.json")).json()).videos||[]}catch{$=[]}let t=h("#pl-subtab-body");t&&g==="music"&&(t.innerHTML=D($))}function D(t){let s=`
    ...
    </div>`;return t.length?_==="grid"?s+H(t):_==="list"?s+tt(t):_==="category"?s+st(t):s+H(t):`${s}<div class="pl-empty-state"><p>\u52D5\u753B\u304C\u767B\u9332\u3055\u308C\u3066\u3044\u307E\u305B\u3093</p><p class="pl-empty-hint">\u7BA1\u7406\u753B\u9762\u304B\u3089\u767B\u9332\u3067\u304D\u307E\u3059</p></div>`}function j(t){switch(t.type){case"cover":return{label:"\u30AB\u30D0\u30FC",cls:"mv-badge-cover",sub:t.originalArtist||"\u30AB\u30D0\u30FC\u66F2"};case"office":return{label:"\u4E8B\u52D9\u6240",cls:"mv-badge-office",sub:"\u4E8B\u52D9\u6240\u30AA\u30EA\u66F2"};case"character":return{label:"\u30AD\u30E3\u30E9",cls:"mv-badge-character",sub:t.character||"\u30AD\u30E3\u30E9\u30BD\u30F3"};default:return{label:"\u30AA\u30EA\u30B8\u30CA\u30EB",cls:"mv-badge-original",sub:"\u304B\u306A\u3046"}}}function C(t,s){let a=L(t.url),l=B(t.url),{label:e,cls:d,sub:b}=j(t);return`
    <div class="mv-card">
    ...
    <div class="mv-list-row">
      ...
      <button class="mv-add-btn" type="button"
        ...
        data-stream-title="${c(t.title||"")}"
        title="\u30D7\u30EC\u30A4\u30EA\u30B9\u30C8\u306B\u8FFD\u52A0">\uFF0B</button>
    </div>`}function H(t){return`<div class="mv-grid">${t.map((s,a)=>C(s,a)).join("")}</div>`}function tt(t){return`<div class="mv-list">${t.map((s,a)=>X(s,a)).join("")}</div>`}function st(t){let s=t.map((l,e)=>({v:l,i:e})).filter(({v:l})=>l.type==="original"),a=t.map((l,e)=>({v:l,i:e})).filter(({v:l})=>l.type==="cover");return`
    <div class="mv-category">
      <div class="mv-cat-section">
        <h3 class="mv-cat-heading">\u30AA\u30EA\u30B8\u30CA\u30EB\u66F2 <span class="mv-cat-count">${s.length}</span></h3>
        ${s.length?`<div class="mv-grid">${s.map(({v:l,i:e})=>C(l,e)).join("")}</div>`:'<p class="mv-cat-empty">\u306A\u3057</p>'}
      </div>
      <div class="mv-cat-section">
        <h3 class="mv-cat-heading">\u30AB\u30D0\u30FC\u66F2\uFF08\u6B4C\u307F\u305F\uFF09 <span class="mv-cat-count">${a.length}</span></h3>
        ${a.length?`<div class="mv-grid">${a.map(({v:l,i:e})=>C(l,e)).join("")}</div>`:'<p class="mv-cat-empty">\u306A\u3057</p>'}
      </div>
    ...
    <div class="pl-grid">
      ...
      <div class="pl-my-actions">
        <button class="pl-new-btn" id="pl-new-btn" type="button">\uFF0B \u65B0\u898F\u4F5C\u6210</button>
      </div>`}function lt(t,s){let a=t.streams.map(r=>{let n=r.startsWith("mv:"),i=n?at(r):null;return{skey:r,isMv:n,mv:i,stream:n?null:s.find(o=>S(o)===r)}}),l=a.find(({stream:r,mv:n})=>r?.url||n?.url)?.stream?.url||a.find(({mv:r})=>r?.url)?.mv?.url,e=l?`<img class="pl-card-cover" src="${c(L(l))}" alt="" loading="lazy" referrerpolicy="no-referrer">`:"",d=a.length,b=a.map(({skey:r,isMv:n,mv:i,stream:o},f)=>{let u=c(t.id+"|:|"+r),p=`
      <div class="pl-sort-btns">
        ...
        </div>`;let{label:P,sub:F}=j(i),G=i.type||"original",A=($||[]).indexOf(i);return`
        <div class="pl-stream-row">
          ${p}
          <div class="pl-stream-info">
            <span class="pl-stream-date"><span class="mv-badge-inline mv-type-${G}">${P}</span></span>
            <span class="pl-stream-title">${c(i.title||"\u2014")}</span>
            <span class="pl-stream-meta">${c(F)}</span>
          </div>
          <div class="pl-stream-actions">
            ${A>=0?`<button class="pl-play-stream-btn" data-play-music-pl="${A}" type="button" title="\u518D\u751F">\u25B6</button>`:""}
            ${m}
          </div>
        </div>`}return o?`
      <div class="pl-stream-row">
        ${p}
        <div class="pl-stream-info">
          <span class="pl-stream-date">${T(o.date)}</span>

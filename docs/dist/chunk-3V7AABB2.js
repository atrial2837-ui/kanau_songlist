import{a as nt,e as it,f as G,g as at,h as ot,j as rt,l as w}from"./chunk-FIQJD23O.js";import{a as Z,b as D,e}from"./chunk-JT7WCFD3.js";import{B as K,D as st,F as R,L as et,O as j,a as u,c as r,g as tt,k as q,l as Q,m as _}from"./chunk-T4BEBXYH.js";var $,T,k,V,Y,H,ut,P,ft=null,gt="kanau-setlist-v1",x=[],M=null,I=null,E=null;function W(){E&&(E.disconnect(),E=null)}function vt(t){if(W(),e.songsLimit>=t)return;let s=document.getElementById("songs-infinite-sentinel");s&&(E=new IntersectionObserver(i=>{i[0].isIntersecting&&(e.songsLimit+=100,y())},{rootMargin:"200px"}),E.observe(s))}function Yt(){W(),kt(),Et(),nt(e.data?.songs||[]);let t=u("#panel-songs");t.innerHTML=`
  ...
  `,$=u("#songs-search"),T=u("#songs-sort"),k=u("#songs-genre"),V=u("#songs-filters"),Y=u("#songs-genre-chips"),H=u("#songs-list"),ut=u("#songs-count"),P=u("#songs-more-wrap"),T.value=e.songsSort,k.value=bt(e.songsGenre)?e.songsGenre:"all",e.songsGenre=k.value,O(),N(),U();let s=document.getElementById("search-suggest"),i=et(()=>{e.songsQuery=$.value,e.songsLimit=100,G(e.songsQuery),F(),w({tab:"songs",q:e.songsQuery},{replace:!0}),N(),y()},120);$.addEventListener("input",()=>{i()}),$.addEventListener("focus",()=>{lt()}),$.addEventListener("blur",()=>{setTimeout(()=>{F()},200)}),s&&s.addEventListener("click",n=>{let l=n.target.closest("[data-suggest]");if(!l)return;let c=l.dataset.suggest;$.value===c?($.value="",e.songsQuery=""):($.value=c,e.songsQuery=c),e.songsLimit=100,G(e.songsQuery||c),w({tab:"songs",q:e.songsQuery},{replace:!0}),N(),y()}),T.addEventListener("change",()=>{e.songsSort=T.value,y()}),k.addEventListener("change",()=>{e.songsGenre=k.value,e.songsLimit=100,U(),y()}),V.addEventListener("click",n=>{let l=n.target.closest("[data-filter]");l&&(l.dataset.filter==="favorites"?e.favoritesFilter=!e.favoritesFilter:(e.songsFilter=l.dataset.filter,e.favoritesFilter=!1),e.songsLimit=100,O(),y())}),t.querySelector(".songs-fav-toggle")?.addEventListener("click",()=>{e.favoritesFilter=!e.favoritesFilter,e.favoritesFilter&&(e.songsFilter="all"),e.songsLimit=100,O(),y()}),Y.addEventListener("click",n=>{let l=n.target.closest("[data-genre]");l&&(e.songsGenre=l.dataset.genre,k.value=e.songsGenre,e.songsLimit=100,U(),y())});for(let n of t.querySelectorAll("[data-singer-preset]"))n.addEventListener("click",()=>{e.singerMode=!0,e.singerPreset=e.singerPreset===n.dataset.singerPreset?"all":n.dataset.singerPreset,e.songsLimit=100,y()});u("#compact-btn")?.addEventListener("click",()=>{e.songsView=e.songsView==="compact"?"comfortable":"compact",y()}),u("#setlist-toggle-btn")?.addEventListener("click",()=>ht()),u("#recommend-btn")?.addEventListener("click",()=>wt()),u("#todays-song-btn")?.addEventListener("click",()=>dt());for(let n of t.querySelectorAll("[data-mobile-panel-toggle]"))n.addEventListener("click",()=>yt(n.dataset.mobilePanelToggle));t.onclick=n=>{if(n.target.closest("#search-history-clear")){n.preventDefault(),n.stopPropagation(),ot(),F();return}let c=n.target.closest(".search-history-remove");if(c){n.preventDefault(),n.stopPropagation(),at(c.dataset.remove),lt();return}let o=n.target.closest(".search-history-item");if(o){n.preventDefault(),n.stopPropagation();let f=o.dataset.query;e.songsQuery=f,$.value=f,e.songsLimit=100,F(),w({tab:"songs",q:f}),y();return}if(n.target.closest("[data-recommend-dismiss]")){n.preventDefault(),n.stopPropagation();let f=u("#recommend-box");f&&(f.hidden=!0,f.innerHTML="");return}if(n.target.closest("[data-todays-song-dismiss]")){n.preventDefault(),n.stopPropagation();let f=u("#todays-song-box");f&&(f.hidden=!0,f.innerHTML="");return}if(n.target.closest("[data-todays-song-reroll]")){n.preventDefault(),n.stopPropagation(),dt();return}let p=n.target.closest("[data-setlist-action]");if(p){n.stopPropagation(),It(p);return}let h=n.target.closest("[data-artist-search]");if(h){n.stopPropagation();let f=String(h.dataset.artistSearch||"").replace(/"/g,"");e.songsQuery=`artist:"${f}"`,$.value=e.songsQuery,e.songsLimit=100,w({tab:"songs",q:e.songsQuery}),y();return}let m=n.target.closest("[data-fav-toggle]");if(m){n.preventDefault(),n.stopPropagation();let f=m.dataset.favToggle;Z(f);let B=D(f);m.classList.toggle("is-active",B),m.setAttribute("aria-pressed",String(B)),m.textContent=B?"\u2665":"\u2661";return}let b=n.target.closest("[data-tag-search]");if(!b)return;n.stopPropagation();let L=b.dataset.tagType||"tag";e.songsQuery=`${L}:${b.dataset.tagSearch}`,$.value=e.songsQuery,e.songsLimit=100,w({tab:"songs",q:e.songsQuery}),y()},t.oninput=n=>{n.target.id==="setlist-theme"&&(e.setlist.theme=n.target.value,S())},t.onchange=n=>{n.target.id==="setlist-copy-format"&&(e.setlist.copyFormat=n.target.value,S())},t.onkeydown=n=>{n.key==="Enter"&&(!n.target.closest(".setlist-custom-add")&&!n.target.closest(".setlist-custom-details")||n.target.tagName!=="BUTTON"&&(n.preventDefault(),pt()))},y()}function lt(){let t=it(),s=u("#search-history-dropdown");s&&(ft=s,t.length?s.innerHTML=`
      ...
      `).join("")}
    `:s.innerHTML='<div class="search-history-empty">\u691C\u7D22\u5C65\u6B74\u304C\u3042\u308A\u307E\u305B\u3093</div>',s.hidden=!1)}function F(){let t=u("#search-history-dropdown");t&&(t.hidden=!0)}function yt(t){let s=u("#songs-filter-panel"),i=u("#setlist-planner");if(t==="setlist"&&!e.singerMode){s?.classList.add("is-open"),i?.classList.remove("is-open");for(let l of document.querySelectorAll("[data-mobile-panel-toggle]"))l.classList.toggle("active",l.dataset.mobilePanelToggle==="filters");return}if(e.singerMode){s?.classList.add("is-open"),s?.scrollIntoView({behavior:"smooth",block:"start"});for(let c of document.querySelectorAll("[data-mobile-panel-toggle]"))c.classList.toggle("active",c.dataset.mobilePanelToggle==="filters");return}let n=t==="setlist";s?.classList.toggle("is-open",!n),i?.classList.toggle("is-open",n);for(let l of document.querySelectorAll("[data-mobile-panel-toggle]"))l.classList.toggle("active",l.dataset.mobilePanelToggle===t)}function ht(){if(!e.singerMode)return;e.setlistExpanded=!e.setlistExpanded,v();let t=u("#setlist-planner");e.setlistExpanded&&t?.scrollIntoView({behavior:"smooth",block:"start"})}function A(t){return String(t.genre||"\u672A\u5206\u985E").trim()||"\u672A\u5206\u985E"}function z(){let t=new Map;for(let s of e.data.songs||[]){let i=A(s);t.set(i,(t.get(i)||0)+1)}return[...t.entries()].sort((s,i)=>i[1]-s[1]||s[0].localeCompare(i[0],"ja"))}function bt(t){return t==="all"||z().some(([s])=>s===t)}function $t(){let t=['<option value="all">\u5168\u30B8\u30E3\u30F3\u30EB</option>'];for(let[s,i]of z())t.push(`<option value="${r(s)}">${r(s)} (${i})</option>`);return t.join("")}function Lt(){let t=['<button class="genre-chip" type="button" data-genre="all">\u5168\u30B8\u30E3\u30F3\u30EB</button>'];for(let[s,i]of z())t.push(`
      <button class="genre-chip" type="button" data-genre="${r(s)}">
        <span>${r(s)}</span><small>${i}</small>
      </button>
    `);return t.join("")}function U(){for(let t of Y.querySelectorAll("[data-genre]"))t.classList.toggle("active",t.dataset.genre===e.songsGenre)}function O(){for(let s of V.querySelectorAll("[data-filter]"))s.dataset.filter==="favorites"?(s.classList.toggle("primary",e.favoritesFilter),s.classList.toggle("ghost",!e.favoritesFilter)):(s.classList.toggle("primary",s.dataset.filter===e.songsFilter&&!e.favoritesFilter),s.classList.toggle("ghost",s.dataset.filter!==e.songsFilter||e.favoritesFilter));let t=document.querySelector(".songs-fav-toggle");t&&(t.classList.toggle("is-active",e.favoritesFilter),t.setAttribute("aria-pressed",String(e.favoritesFilter)),t.textContent=e.favoritesFilter?"\u2665":"\u2661")}function N(){let t=document.getElementById("search-suggest");if(!t)return;let s=(e.songsQuery||"").trim();for(let i of t.querySelectorAll("[data-suggest]"))i.classList.toggle("is-active",i.dataset.suggest===s)}function y(){let{songs:t}=e.data,s=q(t,e.songsGenre,A),i=_(s,{singerMode:e.singerMode,preset:e.singerPreset,keyPublished:e.data?.stats?.keyPublished}),n=Q(i,e.songsFilter),{results:l,tokens:c}=rt(e.songsQuery,n),o=e.songsQuery.trim()?l.filter(a=>n.includes(a)):n;if(e.favoritesFilter&&(o=o.filter(a=>e.favorites.has(a.key))),o=K(o,e.songsSort,!!e.songsQuery.trim()),x=o,ut.textContent=`${o.length} / ${t.length}\u66F2`,!o.length){H.innerHTML='<div class="empty-state">\u8A72\u5F53\u3059\u308B\u66F2\u304C\u3042\u308A\u307E\u305B\u3093 \u{1F420}</div>',P.innerHTML="";return}let d=o.slice(0,e.songsLimit);H.classList.toggle("compact",e.songsView==="compact");for(let a of document.querySelectorAll("[data-singer-preset]")){let g=e.singerMode&&e.singerPreset===a.dataset.singerPreset;a.classList.toggle("primary",g),a.classList.toggle("ghost",!g)}u("#compact-btn")&&(u("#compact-btn").textContent=`\u8868\u793A: ${e.songsView==="compact"?"\u30B3\u30F3\u30D1\u30AF\u30C8":"\u8A73\u7D30"}`),H.innerHTML=d.map(a=>Qt(a,c)).join(""),v(),e.songsLimit<o.length?(P.innerHTML=`
  ...
  `}function kt(){try{let t=localStorage.getItem(gt);if(!t)return;let s=JSON.parse(t);e.setlist.theme=String(s.theme||""),e.setlist.copyFormat=s.copyFormat==="timestamp"?"timestamp":"simple",e.setlist.items=Array.isArray(s.items)?s.items:[]}catch{e.setlist.items=[]}}function xt(){let t=e.setlist.items;if(!t.length)return window.location.href.split("?")[0];let s=btoa(unescape(encodeURIComponent(JSON.stringify(t)))),i=new URL(window.location.href.split("?")[0]);return i.searchParams.set("setlist",s),i.toString()}function Et(){try{let s=new URLSearchParams(window.location.search).get("setlist");if(!s)return;let i=decodeURIComponent(escape(atob(s))),n=JSON.parse(i);if(!Array.isArray(n)||!n.length)return;let l=new Set(e.setlist.items.map(o=>o.key)),c=n.filter(o=>!l.has(o.key));c.length&&(e.setlist.items=[...e.setlist.items,...c],S())}catch{}}async function Tt(){let t=xt();if(!e.setlist.items.length){v("\u5171\u6709\u3059\u308B\u66F2\u304C\u3042\u308A\u307E\u305B\u3093");return}try{await navigator.clipboard.writeText(t),v("\u5171\u6709URL\u3092\u30B3\u30D4\u30FC\u3057\u307E\u3057\u305F")}catch{v("\u30B3\u30D4\u30FC\u306B\u5931\u6557\u3057\u307E\u3057\u305F")}}function S(){localStorage.setItem(gt,JSON.stringify(e.setlist))}function J(t){return(e.data.songs||[]).find(s=>s.key===t)||null}function C(t){t&&(e.setlist.items.push({key:t.key,title:t.title,artist:t.artist,displayKey:t.displayKey||"",genre:t.genre||"",moodTags:t.moodTags||[],seasonTags:t.seasonTags||[],daysSinceLast:t.daysSinceLast}),S(),v("\u8FFD\u52A0\u3057\u307E\u3057\u305F"))}function pt(){let t=u("#setlist-custom-title"),s=u("#setlist-custom-artist"),i=u("#setlist-custom-key"),n=String(t?.value||"").trim(),l=String(s?.value||"").trim(),c=String(i?.value||"").trim();if(!n){v("\u66F2\u540D\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044");return}e.setlist.items.push({key:`custom:${Date.now()}:${Math.random().toString(36).slice(2,8)}`,custom:!0,title:n,artist:l,displayKey:c,genre:"\u65B0\u898F",moodTags:[],seasonTags:[],daysSinceLast:null}),S(),v("\u65B0\u3057\u3044\u66F2\u3092\u8FFD\u52A0\u3057\u307E\u3057\u305F")}function Mt(t){if(t.custom)return t;let s=J(t.key);return s?{...t,...s}:t}function It(t){let s=t.dataset.setlistAction,i=Number(t.dataset.index);if(s==="add"&&C(J(t.dataset.songkey)),s==="todays-song-add"&&C(J(t.dataset.songkey)),s==="remove"&&e.setlist.items.splice(i,1),s==="up"&&i>0&&([e.setlist.items[i-1],e.setlist.items[i]]=[e.setlist.items[i],e.setlist.items[i-1]]),s==="down"&&i<e.setlist.items.length-1&&([e.setlist.items[i+1],e.setlist.items[i]]=[e.setlist.items[i],e.setlist.items[i+1]]),s==="copy-item"){qt(i);return}if(s==="add-custom"){pt();return}if(s==="random"&&Ft(),s==="copy"&&Dt(),s==="share"){Tt();return}s==="clear"&&confirm("\u30BB\u30C8\u30EA\u3092\u7A7A\u306B\u3057\u307E\u3059\u304B\uFF1F")&&(e.setlist.items=[]),S(),["add","random","copy"].includes(s)||v()}function Ft(){let t=new Set(e.setlist.items.map(n=>n.key)),s=(x.length?x:e.data.songs).filter(n=>n.key&&!t.has(n.key));if(!s.length){v("\u8FFD\u52A0\u3067\u304D\u308B\u5019\u88DC\u304C\u3042\u308A\u307E\u305B\u3093");return}let i=s[Math.floor(Math.random()*s.length)];C(i)}function X(){return e.setlist.items.map(Mt)}function Ht(t){let s=i=>{let n=new Map;for(let l of t)for(let c of i(l))c&&n.set(c,(n.get(c)||0)+1);return[...n.entries()].sort((l,c)=>c[1]-l[1]).slice(0,3)};return{genres:s(i=>[i.genre||"\u672A\u5206\u985E"]),moods:s(i=>i.moodTags||[]),keys:t.filter(i=>i.displayKey).length,stale:t.filter(i=>i.daysSinceLast>=180).length}}function v(t=""){let s=u("#setlist-planner");if(!s)return;if(_t(),s.hidden=!e.singerMode||!e.setlistExpanded,s.classList.toggle("is-open",e.singerMode&&e.setlistExpanded),!e.singerMode){s.innerHTML="";return}let i=X(),n=Ht(i),l=i.length*5;s.innerHTML=`
    ...
    <div class="setlist-items">
      ${i.length?i.map((c,o)=>Pt(c,o)).join(""):'<div class="setlist-empty">\u66F2\u306E\u300C\uFF0B\u30BB\u30C8\u30EA\u300D\u304B\u30E9\u30F3\u30C0\u30E0\u8FFD\u52A0\u304B\u3089\u4F5C\u308C\u307E\u3059</div>'}
    </div>
  ...
  `,Ct(),At()}function ct(t,s){return s.length?`<span>${t} ${s.map(([i,n])=>`${r(i)} ${n}`).join(" / ")}</span>`:`<span>${t} \u2014</span>`}function Pt(t,s){return`

import{b as v,c as h}from"./chunk-MKJIXTK4.js";import{J as b,L as g,e as i,f as d,i as r}from"./chunk-5JCHYUC5.js";function f(){let{streams:n}=i.data,o=i.timelineFilter,s=o?n.filter(t=>t.songs.some(l=>l.key===o.key)):n,a=y(s,i.timelineSort),c=d("#panel-timeline");c.innerHTML=`
  ...
  `,d("#timeline-sort")?.addEventListener("change",t=>{i.timelineSort=t.target.value||"date-desc",i.timelineLimit=v,f()});let p=d("#timeline-filter-banner");if(o){let t=a.reduce((l,u)=>l+u.songs.filter($=>$.key===o.key).length,0);p.innerHTML=`
    ...
    `,d("#clear-filter").addEventListener("click",()=>{i.timelineFilter=null,i.timelineLimit=v,f()})}if(!a.length){d("#timeline").innerHTML='<div class="empty-state">\u8A72\u5F53\u3059\u308B\u914D\u4FE1\u304C\u3042\u308A\u307E\u305B\u3093 \u{1F420}</div>';return}let m=a.slice(0,i.timelineLimit);if(d("#timeline").innerHTML=m.map((t,l)=>k(t,l,o)).join(""),i.timelineFocus){let l=document.querySelector(`[data-streamkey="${CSS.escape(i.timelineFocus)}"]`)?.closest(".timeline-item");l?.classList.add("focus"),l?.scrollIntoView({behavior:"smooth",block:"center"}),i.timelineFocus=null}d("#timeline").onclick=async t=>{let l=t.target.closest("[data-copy-stream]");if(!l)return;t.stopPropagation();let u=m[Number(l.dataset.copyStream)];if(u)try{await navigator.clipboard.writeText(S(u)),l.textContent="\u30B3\u30D4\u30FC\u6E08\u307F",setTimeout(()=>{l.textContent="\u30BB\u30C8\u30EA\u30B3\u30D4\u30FC"},1200)}catch{l.textContent="\u5931\u6557",setTimeout(()=>{l.textContent="\u30BB\u30C8\u30EA\u30B3\u30D4\u30FC"},1200)}};let e=d("#timeline-controls");i.timelineLimit<a.length&&(e.innerHTML=`<button class="load-more-btn" id="load-more">\u25BC \u3082\u3063\u3068\u898B\u308B (\u6B8B\u308A${a.length-i.timelineLimit}\u67A0)</button>`,d("#load-more").addEventListener("click",()=>{i.timelineLimit+=h,f()}))}function k(n,o,s){let a=!s&&i.timelineSort==="date-desc"&&o<3?"recent":"",c=n.songs.map((t,l)=>{let u=s&&t.key===s.key?" hit":"",$=u?"\u30AF\u30EA\u30C3\u30AF\u3067\u7D5E\u308A\u8FBC\u307F\u89E3\u9664":"\u30AF\u30EA\u30C3\u30AF\u3067\u7D5E\u308A\u8FBC\u307F";return`<span class="setlist-song${u}" data-songkey="${r(t.key)}" data-songtitle="${r(t.title)}" data-songartist="${r(t.artist)}" title="${$}"><span class="sl-num">${l+1}</span>${r(t.title)}<span style="color:var(--ink-mute);"> / ${r(t.artist)}</span></span>`}).join(""),p=n.url?`<a href="${r(n.url)}" target="_blank" rel="noopener">${r(n.title||"\u914D\u4FE1")}</a>`:r(n.title||"\u914D\u4FE1"),m=n.url?`<span class="watch-actions"><button class="watch-link" type="button" data-stream-play="${r(g(n))}" data-inline-youtube="${r(n.url)}">\u25B6 \u518D\u751F</button><a class="watch-open-link" href="${r(n.url)}" target="_blank" rel="noopener">\u2197 \u958B\u304F</a></span>`:"",e=`<button class="timeline-copy-btn" type="button" data-copy-stream="${o}">\u30BB\u30C8\u30EA\u30B3\u30D4\u30FC</button>`;return`
    <article class="timeline-item ${a}">
      <span class="stream-anchor" data-streamkey="${r(g(n))}"></span>
      <header class="timeline-head">
        <span class="timeline-date">${b(n.date)}</span>

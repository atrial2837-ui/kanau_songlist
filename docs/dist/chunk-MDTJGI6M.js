import{a as et,e as nt,f as G,g as it,h as at,j as ot,l as S}from"./chunk-TKHIMASD.js";import{a as X,b as D,e}from"./chunk-JT7WCFD3.js";import{A as K,C as tt,E as R,K as st,N as j,a as u,d as r,h as Z,k as q,l as Q,m as _}from"./chunk-PRFEE4R6.js";var $,T,w,N,V,P,ut,C,ft=null,gt="kanau-setlist-v1",k=[],M=null,I=null,x=null;function J(){x&&(x.disconnect(),x=null)}function yt(t){if(J(),e.songsLimit>=t)return;let s=document.getElementById("songs-infinite-sentinel");s&&(x=new IntersectionObserver(i=>{i[0].isIntersecting&&(e.songsLimit+=100,v())},{rootMargin:"200px"}),x.observe(s))}function Yt(){J(),kt(),Et(),et(e.data?.songs||[]);let t=u("#panel-songs");t.innerHTML=`
  ...
  `,$=u("#songs-search"),T=u("#songs-sort"),w=u("#songs-genre"),N=u("#songs-filters"),V=u("#songs-genre-chips"),P=u("#songs-list"),ut=u("#songs-count"),C=u("#songs-more-wrap"),T.value=e.songsSort,w.value=bt(e.songsGenre)?e.songsGenre:"all",e.songsGenre=w.value,lt(),O(),U();let s=document.getElementById("search-suggest"),i=st(()=>{e.songsQuery=$.value,e.songsLimit=100,G(e.songsQuery),H(),S({tab:"songs",q:e.songsQuery},{replace:!0}),O(),v()},120);$.addEventListener("input",()=>{i()}),$.addEventListener("focus",()=>{rt()}),$.addEventListener("blur",()=>{setTimeout(()=>{H()},200)}),s&&s.addEventListener("click",n=>{let l=n.target.closest("[data-suggest]");if(!l)return;let c=l.dataset.suggest;$.value===c?($.value="",e.songsQuery=""):($.value=c,e.songsQuery=c),e.songsLimit=100,G(e.songsQuery||c),S({tab:"songs",q:e.songsQuery},{replace:!0}),O(),v()}),T.addEventListener("change",()=>{e.songsSort=T.value,v()}),w.addEventListener("change",()=>{e.songsGenre=w.value,e.songsLimit=100,U(),v()}),N.addEventListener("click",n=>{let l=n.target.closest("[data-filter]");l&&(l.dataset.filter==="favorites"?e.favoritesFilter=!e.favoritesFilter:(e.songsFilter=l.dataset.filter,e.favoritesFilter=!1),e.songsLimit=100,lt(),v())}),V.addEventListener("click",n=>{let l=n.target.closest("[data-genre]");l&&(e.songsGenre=l.dataset.genre,w.value=e.songsGenre,e.songsLimit=100,U(),v())});for(let n of t.querySelectorAll("[data-singer-preset]"))n.addEventListener("click",()=>{e.singerMode=!0,e.singerPreset=e.singerPreset===n.dataset.singerPreset?"all":n.dataset.singerPreset,e.songsLimit=100,v()});u("#compact-btn")?.addEventListener("click",()=>{e.songsView=e.songsView==="compact"?"comfortable":"compact",v()}),u("#setlist-toggle-btn")?.addEventListener("click",()=>ht()),u("#recommend-btn")?.addEventListener("click",()=>wt()),u("#todays-song-btn")?.addEventListener("click",()=>dt());for(let n of t.querySelectorAll("[data-mobile-panel-toggle]"))n.addEventListener("click",()=>vt(n.dataset.mobilePanelToggle));t.onclick=n=>{if(n.target.closest("#search-history-clear")){n.preventDefault(),n.stopPropagation(),at(),H();return}let c=n.target.closest(".search-history-remove");if(c){n.preventDefault(),n.stopPropagation(),it(c.dataset.remove),rt();return}let d=n.target.closest(".search-history-item");if(d){n.preventDefault(),n.stopPropagation();let f=d.dataset.query;e.songsQuery=f,$.value=f,e.songsLimit=100,H(),S({tab:"songs",q:f}),v();return}if(n.target.closest("[data-recommend-dismiss]")){n.preventDefault(),n.stopPropagation();let f=u("#recommend-box");f&&(f.hidden=!0,f.innerHTML="");return}if(n.target.closest("[data-todays-song-dismiss]")){n.preventDefault(),n.stopPropagation();let f=u("#todays-song-box");f&&(f.hidden=!0,f.innerHTML="");return}if(n.target.closest("[data-todays-song-reroll]")){n.preventDefault(),n.stopPropagation(),dt();return}let m=n.target.closest("[data-setlist-action]");if(m){n.stopPropagation(),It(m);return}let h=n.target.closest("[data-artist-search]");if(h){n.stopPropagation();let f=String(h.dataset.artistSearch||"").replace(/"/g,"");e.songsQuery=`artist:"${f}"`,$.value=e.songsQuery,e.songsLimit=100,S({tab:"songs",q:e.songsQuery}),v();return}let g=n.target.closest("[data-fav-toggle]");if(g){n.preventDefault(),n.stopPropagation();let f=g.dataset.favToggle;X(f);let B=D(f);g.classList.toggle("is-active",B),g.setAttribute("aria-pressed",String(B)),g.textContent=B?"\u2665":"\u2661";return}let b=n.target.closest("[data-tag-search]");if(!b)return;n.stopPropagation();let E=b.dataset.tagType||"tag";e.songsQuery=`${E}:${b.dataset.tagSearch}`,$.value=e.songsQuery,e.songsLimit=100,S({tab:"songs",q:e.songsQuery}),v()},t.oninput=n=>{n.target.id==="setlist-theme"&&(e.setlist.theme=n.target.value,L())},t.onchange=n=>{n.target.id==="setlist-copy-format"&&(e.setlist.copyFormat=n.target.value,L())},t.onkeydown=n=>{n.key==="Enter"&&(!n.target.closest(".setlist-custom-add")&&!n.target.closest(".setlist-custom-details")||n.target.tagName!=="BUTTON"&&(n.preventDefault(),pt()))},v()}function rt(){let t=nt(),s=u("#search-history-dropdown");s&&(ft=s,t.length?s.innerHTML=`
  ...
  `}function kt(){try{let t=localStorage.getItem(gt);if(!t)return;let s=JSON.parse(t);e.setlist.theme=String(s.theme||""),e.setlist.copyFormat=s.copyFormat==="timestamp"?"timestamp":"simple",e.setlist.items=Array.isArray(s.items)?s.items:[]}catch{e.setlist.items=[]}}function xt(){let t=e.setlist.items;if(!t.length)return window.location.href.split("?")[0];let s=btoa(unescape(encodeURIComponent(JSON.stringify(t)))),i=new URL(window.location.href.split("?")[0]);return i.searchParams.set("setlist",s),i.toString()}function Et(){try{let s=new URLSearchParams(window.location.search).get("setlist");if(!s)return;let i=decodeURIComponent(escape(atob(s))),n=JSON.parse(i);if(!Array.isArray(n)||!n.length)return;let l=new Set(e.setlist.items.map(d=>d.key)),c=n.filter(d=>!l.has(d.key));c.length&&(e.setlist.items=[...e.setlist.items,...c],L())}catch{}}async function Tt(){let t=xt();if(!e.setlist.items.length){y("\u5171\u6709\u3059\u308B\u66F2\u304C\u3042\u308A\u307E\u305B\u3093");return}try{await navigator.clipboard.writeText(t),y("\u5171\u6709URL\u3092\u30B3\u30D4\u30FC\u3057\u307E\u3057\u305F")}catch{y("\u30B3\u30D4\u30FC\u306B\u5931\u6557\u3057\u307E\u3057\u305F")}}function L(){localStorage.setItem(gt,JSON.stringify(e.setlist))}function Y(t){return(e.data.songs||[]).find(s=>s.key===t)||null}function F(t){t&&(e.setlist.items.push({key:t.key,title:t.title,artist:t.artist,displayKey:t.displayKey||"",genre:t.genre||"",moodTags:t.moodTags||[],seasonTags:t.seasonTags||[],daysSinceLast:t.daysSinceLast}),L(),y("\u8FFD\u52A0\u3057\u307E\u3057\u305F"))}function pt(){let t=u("#setlist-custom-title"),s=u("#setlist-custom-artist"),i=u("#setlist-custom-key"),n=String(t?.value||"").trim(),l=String(s?.value||"").trim(),c=String(i?.value||"").trim();if(!n){y("\u66F2\u540D\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044");return}e.setlist.items.push({key:`custom:${Date.now()}:${Math.random().toString(36).slice(2,8)}`,custom:!0,title:n,artist:l,displayKey:c,genre:"\u65B0\u898F",moodTags:[],seasonTags:[],daysSinceLast:null}),L(),y("\u65B0\u3057\u3044\u66F2\u3092\u8FFD\u52A0\u3057\u307E\u3057\u305F")}function Mt(t){if(t.custom)return t;let s=Y(t.key);return s?{...t,...s}:t}function It(t){let s=t.dataset.setlistAction,i=Number(t.dataset.index);if(s==="add"&&F(Y(t.dataset.songkey)),s==="todays-song-add"&&F(Y(t.dataset.songkey)),s==="remove"&&e.setlist.items.splice(i,1),s==="up"&&i>0&&([e.setlist.items[i-1],e.setlist.items[i]]=[e.setlist.items[i],e.setlist.items[i-1]]),s==="down"&&i<e.setlist.items.length-1&&([e.setlist.items[i+1],e.setlist.items[i]]=[e.setlist.items[i],e.setlist.items[i+1]]),s==="copy-item"){qt(i);return}if(s==="add-custom"){pt();return}if(s==="random"&&Ht(),s==="copy"&&Dt(),s==="share"){Tt();return}s==="clear"&&confirm("\u30BB\u30C8\u30EA\u3092\u7A7A\u306B\u3057\u307E\u3059\u304B\uFF1F")&&(e.setlist.items=[]),L(),["add","random","copy"].includes(s)||y()}function Ht(){let t=new Set(e.setlist.items.map(n=>n.key)),s=(k.length?k:e.data.songs).filter(n=>n.key&&!t.has(n.key));if(!s.length){y("\u8FFD\u52A0\u3067\u304D\u308B\u5019\u88DC\u304C\u3042\u308A\u307E\u305B\u3093");return}let i=s[Math.floor(Math.random()*s.length)];F(i)}function z(){return e.setlist.items.map(Mt)}function Pt(t){let s=i=>{let n=new Map;for(let l of t)for(let c of i(l))c&&n.set(c,(n.get(c)||0)+1);return[...n.entries()].sort((l,c)=>c[1]-l[1]).slice(0,3)};return{genres:s(i=>[i.genre||"\u672A\u5206\u985E"]),moods:s(i=>i.moodTags||[]),keys:t.filter(i=>i.displayKey).length,stale:t.filter(i=>i.daysSinceLast>=180).length}}function y(t=""){let s=u("#setlist-planner");if(!s)return;if(_t(),s.hidden=!e.singerMode||!e.setlistExpanded,s.classList.toggle("is-open",e.singerMode&&e.setlistExpanded),!e.singerMode){s.innerHTML="";return}let i=z(),n=Pt(i),l=i.length*5;s.innerHTML=`
    ...
    <div class="setlist-items">
      ${i.length?i.map((c,d)=>Ct(c,d)).join(""):'<div class="setlist-empty">\u66F2\u306E\u300C\uFF0B\u30BB\u30C8\u30EA\u300D\u304B\u30E9\u30F3\u30C0\u30E0\u8FFD\u52A0\u304B\u3089\u4F5C\u308C\u307E\u3059</div>'}
    </div>
  ...
  `}function Ft(){let t=document.getElementById("setlist-search-input"),s=document.getElementById("setlist-search-dropdown");if(!t||!s)return;let i=[],n=-1;function l(a){let o=a.trim().toLowerCase();if(!o){s.hidden=!0,i=[],n=-1;return}let m=(e.data?.songs||[]).filter(g=>g.title.toLowerCase().includes(o)||(g.artist||"").toLowerCase().includes(o)).sort((g,b)=>{let E=g.title.toLowerCase().startsWith(o)?2:g.title.toLowerCase().includes(o)?1:0,f=b.title.toLowerCase().startsWith(o)?2:b.title.toLowerCase().includes(o)?1:0;return E!==f?f-E:b.count-g.count}).slice(0,8),h={_isNew:!0,title:a.trim()};m.length?(s.innerHTML=m.map((g,b)=>`
          <div class="setlist-dd-item" data-dd-idx="${b}">
            <span class="setlist-dd-icon">\u{1F3B5}</span>
            <div class="setlist-dd-body">
              <div class="setlist-dd-title">${r(g.title)}</div>
              <div class="setlist-dd-meta">${r(g.artist||"\u2014")} \xB7 ${g.count}\u56DE</div>
            </div>
          </div>`).join("")+`<div class="setlist-dd-item setlist-dd-new" data-dd-idx="${m.length}">
          <span class="setlist-dd-plus">\uFF0B</span>
          <div class="setlist-dd-body">
            <div class="setlist-dd-title">\u300C${r(a.trim())}\u300D\u3092\u65B0\u898F\u8FFD\u52A0</div>
            <div class="setlist-dd-meta">\u66F2\u30EA\u30B9\u30C8\u306B\u306A\u3044\u66F2\u3068\u3057\u3066\u8FFD\u52A0</div>
          </div>
        </div>`,i=[...m,h]):(s.innerHTML=`
        ...
        </div>`,i=[h]),n=-1,s.hidden=!1,c()}function c(){s.querySelectorAll("[data-dd-idx]").forEach((a,o)=>a.classList.toggle("is-selected",o===n))}function d(a){let o=i[a];if(o)if(s.hidden=!0,i=[],n=-1,o._isNew){let p=document.querySelector(".setlist-custom-details"),m=document.getElementById("setlist-custom-title");p&&m?(p.open=!0,m.value=o.title,t.value="",document.getElementById("setlist-custom-artist")?.focus()):t.value=""}else t.value="",F(o)}t.addEventListener("input",()=>l(t.value)),t.addEventListener("keydown",a=>{if(s.hidden)return;let o=i.length;a.key==="ArrowDown"?(a.preventDefault(),n=(n+1)%o,c()):a.key==="ArrowUp"?(a.preventDefault(),n=(n-1+o)%o,c()):a.key==="Enter"?(a.preventDefault(),a.stopPropagation(),d(n>=0?n:0)):a.key==="Escape"&&(s.hidden=!0,n=-1)}),s.addEventListener("mousedown",a=>{let o=a.target.closest("[data-dd-idx]");o&&(a.preventDefault(),d(Number(o.dataset.ddIdx)))}),M&&document.removeEventListener("click",M),M=a=>{!t.contains(a.target)&&!s.contains(a.target)&&(s.hidden=!0,n=-1)},document.addEventListener("click",M)}function At(){I&&(I(),I=null);let t=document.querySelector(".setlist-items");if(!t)return;let s=null,i=()=>{s&&(s.rows.forEach(d=>{d.style.transform=""}),s.row.classList.remove("is-dragging"),t.classList.remove("is-drag-active"),s.row.removeEventListener("pointermove",n),s.row.removeEventListener("pointerup",l),s.row.removeEventListener("pointercancel",c),s=null)};function n(d){if(!s)return;d.preventDefault();let a=d.clientY-s.startY;if(!s.moved&&Math.abs(a)<3)return;s.moved=!0,s.row.style.transform=`translateY(${a}px)`;let o=s.mids[s.startIdx]+a,p=0;for(let m=0;m<s.mids.length;m++)m!==s.startIdx&&o>s.mids[m]&&p++;p!==s.targetIdx&&(s.targetIdx=p,s.rows.forEach((m,h)=>{if(h===s.startIdx)return;let g=0;s.startIdx<p&&h>s.startIdx&&h<=p?g=-s.rowH:s.startIdx>p&&h>=p&&h<s.startIdx&&(g=s.rowH),m.style.transform=g?`translateY(${g}px)`:""}))}function l(){if(!s)return;let{startIdx:d,targetIdx:a,moved:o}=s;if(i(),!o||a===d)return;let p=e.setlist.items;if(d<p.length){let[m]=p.splice(d,1);p.splice(a,0,m),L(),y()}}function c(){i()}t.addEventListener("pointerdown",d=>{if(s||!d.target.closest(".setlist-drag-handle"))return;let a=d.target.closest(".setlist-item");if(!a)return;d.preventDefault();let o=Array.from(t.querySelectorAll(".setlist-item")),p=o.indexOf(a);if(p<0)return;let m=o.map(g=>{let b=g.getBoundingClientRect();return b.top+b.height/2}),h=a.getBoundingClientRect();s={rows:o,mids:m,startIdx:p,targetIdx:p,startY:d.clientY,rowH:h.height+(parseFloat(getComputedStyle(t).rowGap||getComputedStyle(t).gap)||0),row:a,moved:!1},a.classList.add("is-dragging"),t.classList.add("is-drag-active");try{a.setPointerCapture(d.pointerId)}catch{}a.addEventListener("pointermove",n,{passive:!1}),a.addEventListener("pointerup",l),a.addEventListener("pointercancel",c)}),I=i}function Bt(){let t=z(),s=[];return e.setlist.theme&&s.push(`# ${e.setlist.theme}`,""),t.forEach(i=>{s.push(mt(i))}),s.join(`
`)}function mt(t){let s=String(t?.title||"").trim(),i=String(t?.artist||"").trim(),n=i?`${s} / ${i}`:s;return e.setlist.copyFormat==="timestamp"?`00:00\u3000${n}\u300000:00`:n}async function Dt(){let t=Bt();if(!t.trim()){y("\u30B3\u30D4\u30FC\u3059\u308B\u66F2\u304C\u3042\u308A\u307E\u305B\u3093");return}try{await navigator.clipboard.writeText(t),y("\u30B3\u30D4\u30FC\u3057\u307E\u3057\u305F")}catch{y("\u30B3\u30D4\u30FC\u306B\u5931\u6557\u3057\u307E\u3057\u305F")}}async function qt(t){let s=z()[t];if(!s){y("\u30B3\u30D4\u30FC\u3059\u308B\u66F2\u304C\u3042\u308A\u307E\u305B\u3093");return}try{await navigator.clipboard.writeText(mt(s)),y("1\u66F2\u30B3\u30D4\u30FC\u3057\u307E\u3057\u305F")}catch{y("\u30B3\u30D4\u30FC\u306B\u5931\u6557\u3057\u307E\u3057\u305F")}}function Qt(t,s){let i=t.rank===1?"r1":t.rank===2?"r2":t.rank===3?"r3":"",n=t.lastSung?`<span class="last-date">${R(t.lastSung)}</span><span class="badge ${Z(t.daysSinceLast)}">${t.daysSinceLast}\u65E5\u524D</span>`:'<span class="last-date">\u5C65\u6B74\u672A\u78BA\u8A8D</span><span class="badge never">\u8981\u78BA\u8A8D</span>',l=j(t.title,s),c=j(t.artist,s),d=tt(t,e.songsQuery),a=D(t.key);return`
    <div class="song-row" data-songkey="${r(t.key)}" data-songtitle="${r(t.title)}" data-songartist="${r(t.artist)}" title="\u30AF\u30EA\u30C3\u30AF\u3067\u66F2\u8A73\u7D30\u3092\u8868\u793A">
      ...
      <div class="info">
        <div class="title song-title-line"><span class="song-title-text">${l}</span><button class="fav-btn ${a?"is-active":""}" type="button" data-fav-toggle="${r(t.key)}" aria-label="\u304A\u6C17\u306B\u5165\u308A" aria-pressed="${a?"true":"false"}" title="\u304A\u6C17\u306B\u5165\u308A">${a?"\u2665":"\u2661"}</button></div>
        ...
        <div class="song-meta-line">
          ...
          ${Kt(t)}
          ${d.map(o=>`<span class="match-badge">${r(o)}\u4E00\u81F4</span>`).join("")}
        </div>
        ${Rt(t)}
      </div>
